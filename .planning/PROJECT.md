...
- AI change summary without grounding — raw hallucination risk unacceptable; v1.2 addresses this with structured-output grounding + Python validation
...
| LLM grounding via ContextBuilder adapter | LLM never sees raw XML — only structured WorkflowDoc/DiffResult context | v1.2 decision — prevents hallucination at the source |
| Single-retry on ValidationError (no critic loop) | Python post-validation catches structural errors better than an LLM critic; single-pass outperforms multi-turn review | v1.2 decision — lower latency, more reliable grounding |
| langchain~=0.3 + langgraph~=1.1 | Stable production versions as of April 2026; `langchain>=1.2` does not exist | v1.2 decision |
| Optional [llm] dependency group | Core CLI must work without LLM deps; pip install alteryx-diff[llm] for AI features | v1.2 decision |

## Current Milestone: v1.2 LLM Documentation

**Goal:** Automatically generate developer-grade workflow documentation and change narratives from the parser's structured output — anchored to parsed data, with zero hallucination tolerance via Python-side grounding validation.

**Target features:**
- `ContextBuilder`: transforms `WorkflowDoc`/`DiffResult` into token-efficient LLM context (no raw XML)
- `DocumentationGraph` (LangGraph v1.1): linear pipeline — analyze_topology → annotate_tools → risk_scan → assemble_doc
- `alteryx-diff document` CLI subcommand — standalone intent doc generation for a single workflow
- `alteryx-diff diff --doc` flag — change record embedded in HTML diff report
- Companion app integration: "Business context" field on first commit; SSE-streamed AI summary in diff view
- Ollama support for offline/air-gapped execution
- RAGAS evaluation harness (faithfulness + factual grounding metrics)
- Optional `[llm]` dependency group — core CLI unaffected when not installed
